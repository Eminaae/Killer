...
public class SimpleAnimationStarter extends JPanel implements ActionListener {
    ...
    public void drawFrame(Graphics g, int frameNumber, int width, int height) {
        ...
        int inset = frameNumber % 50;
        int rectWidth = width, rectHeight = height;
        g.setColor(Color.orange);
        g.fillRect(0, 0, rectWidth, rectHeight); //Oboji background
        g.setColor(Color.yellow); //boja za crtanje pravougaonika
        inset = frameNumber % 15; //za svaki inset djeljiv sa 15, svakih 15 frames se ponavlja ista slika
        rectWidth = width - 2*inset;
        rectHeight = height - 2*inset;
        
        while(rectWidth >= 0 && rectHeight >= 0){ //crta sve manje pravougaonike
        	g.drawRect(inset, inset, rectWidth, rectHeight);
        inset = inset + 20;
        rectWidth = width - 2*inset; //da iyracunamo sirinu pravougaonika koji se nalayi unutar postojeceg
        rectHeight = height - 2*inset;
        }
