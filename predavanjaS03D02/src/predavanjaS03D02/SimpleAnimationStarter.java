...
public class SimpleAnimationStarter extends JPanel implements ActionListener {
    ...
    public void drawFrame(Graphics g, int frameNumber, int width, int height) {
        ...
        int inset = frameNumber % 20;

        int rectWidth = width - 2*inset; // drawing area width minus two insets
        int rectHeight = height - 2*inset; // drawing area height minus two insets

        while (rectWidth >= 0 && rectHeight >= 0) {
   
            g.drawOval(inset, inset, rectWidth, rectHeight);
            inset += 20;       // rectangles are 15 pixels apart
            rectWidth -= 30;
            rectHeight -= 30;
        }
